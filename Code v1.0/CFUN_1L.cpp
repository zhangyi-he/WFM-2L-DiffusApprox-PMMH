...
// version 1.0
// A single locus without missing genotypes
...
List runBPF_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  for (arma::uword i = 0; i < pcl_num; i++) {
    wght_tmp(i) = R::dbinom(smp_cnt(0), smp_siz(0), part_tmp(i), false);
  }
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      ...
      wght_tmp(i) = R::dbinom(smp_cnt(k), smp_siz(k), part_tmp(i), false);
    }
...
double calculateLogLikelihood_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  for (arma::uword i = 0; i < pcl_num; i++) {
    wght(i) = R::dbinom(smp_cnt(0), smp_siz(0), part_pre(i), false);
  }
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    for (arma::uword i = 0; i < pcl_num; i++) {
      ...
      wght(i) = R::dbinom(smp_cnt(k), smp_siz(k), part_pre(i), false);
    }
...
//[[Rcpp::export]]
List runPMMH_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  log_lik_chn(0) = calculateLogLikelihood_arma(sel_cof_chn(0), dom_par, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    if (sel_cof_chn(i) > 1) {
      ...
      log_lik_chn(i) = calculateLogLikelihood_arma(sel_cof_chn(i), dom_par, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num);

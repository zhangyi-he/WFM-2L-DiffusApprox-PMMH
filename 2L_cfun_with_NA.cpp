...
// Calculate the possible unobserved allele counts from the observed allele counts and missingness
// [[Rcpp::export]]
arma::imat calculateAlleleCount_UnphasedChr_arma(const arma::icolvec& smp_ale_cnt, const arma::icolvec& mis_ale_cnt) {
  // ensure RNG gets set/reset
  RNGScope scope;
  
  int smp_ale_cnt_min_A = smp_ale_cnt(0);
  int smp_ale_cnt_max_A = smp_ale_cnt(0) + mis_ale_cnt(0);
  arma::irowvec smp_ale_cnt_rng_A = arma::linspace<arma::irowvec>(smp_ale_cnt_min_A, smp_ale_cnt_max_A, smp_ale_cnt_max_A - smp_ale_cnt_min_A + 1);
  
  int smp_ale_cnt_min_B = smp_ale_cnt(1);
  int smp_ale_cnt_max_B = smp_ale_cnt(1) + mis_ale_cnt(1);
  arma::irowvec smp_ale_cnt_rng_B = arma::linspace<arma::irowvec>(smp_ale_cnt_min_B, smp_ale_cnt_max_B, smp_ale_cnt_max_B - smp_ale_cnt_min_B + 1);
  
  arma::imat ptl_smp_ale_cnt = arma::zeros<arma::imat>(2, smp_ale_cnt_rng_A.n_elem * smp_ale_cnt_rng_B.n_elem);
  for (arma::uword i = 0; i < smp_ale_cnt_rng_A.n_elem; i++) {
    for (arma::uword j = 0; j < smp_ale_cnt_rng_B.n_elem; j++) {
      ptl_smp_ale_cnt(0, j + i * smp_ale_cnt_rng_B.n_elem) = smp_ale_cnt_rng_A(i);
      ptl_smp_ale_cnt(1, j + i * smp_ale_cnt_rng_B.n_elem) = smp_ale_cnt_rng_B(j);
    }
  }
  
  return ptl_smp_ale_cnt;
}

// Calculate the possible unobserved haplotype counts from the possible unobserved allele counts
// [[Rcpp::export]]
arma::imat calculateHaploCount_UnphasedChr_arma(const int& smp_chr_cnt, const arma::icolvec& smp_ale_cnt, const arma::icolvec& mis_ale_cnt) {
  // ensure RNG gets set/reset
  RNGScope scope;
  
  arma::imat ptl_smp_ale_cnt = calculateAlleleCount_UnphasedChr_arma(smp_ale_cnt, mis_ale_cnt);
  
  arma::imat ptl_smp_hap_cnt;
  
  arma::icolvec tmp_smp_ale_cnt = ptl_smp_ale_cnt.col(0);
  if (tmp_smp_ale_cnt(0) + tmp_smp_ale_cnt(1) - smp_chr_cnt > 0) {
    int smp_hap_cnt_min = tmp_smp_ale_cnt(0) + tmp_smp_ale_cnt(1) - smp_chr_cnt;
    int smp_hap_cnt_max = min(tmp_smp_ale_cnt(0), tmp_smp_ale_cnt(1));
    ...
    smp_hap_cnt.row(1) = tmp_smp_ale_cnt(0) - smp_hap_cnt.row(0);
    smp_hap_cnt.row(2) = tmp_smp_ale_cnt(1) - smp_hap_cnt.row(0);
    smp_hap_cnt.row(3) = smp_chr_cnt - smp_hap_cnt.row(0) - smp_hap_cnt.row(1) - smp_hap_cnt.row(2);
    
    ptl_smp_hap_cnt = smp_hap_cnt;
  } else {
  ...
  for (arma::uword i = 1; i < ptl_smp_ale_cnt.n_cols; i++) {
    arma::icolvec tmp_smp_ale_cnt = ptl_smp_ale_cnt.col(i);
    if (tmp_smp_ale_cnt(0) + tmp_smp_ale_cnt(1) - smp_chr_cnt > 0) {
      int smp_hap_cnt_min = tmp_smp_ale_cnt(0) + tmp_smp_ale_cnt(1) - smp_chr_cnt;
      int smp_hap_cnt_max = min(tmp_smp_ale_cnt(0), tmp_smp_ale_cnt(1));
      arma::irowvec smp_hap_cnt_rng = arma::linspace<arma::irowvec>(smp_hap_cnt_min, smp_hap_cnt_max, smp_hap_cnt_max - smp_hap_cnt_min + 1);
      ...
      ptl_smp_hap_cnt.insert_cols(0, smp_hap_cnt);
    } else {
      int smp_hap_cnt_min = 0;
      int smp_hap_cnt_max = min(tmp_smp_ale_cnt(0), tmp_smp_ale_cnt(1));
      arma::irowvec smp_hap_cnt_rng = arma::linspace<arma::irowvec>(smp_hap_cnt_min, smp_hap_cnt_max, smp_hap_cnt_max - smp_hap_cnt_min + 1);
  ...
  return ptl_smp_hap_cnt;
}

// Run the particle filter for the observations with unphased chromosomes
// [[Rcpp::export]]
List runBPF_UnphasedChr_arma(const double& sel_cof_A, const double& dom_par_A, const double& sel_cof_B, const double& dom_par_B, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::imat& smp_ale_cnt, const arma::imat& mis_ale_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  arma::imat ptl_smp_hap_cnt = calculateHaploCount_UnphasedChr_arma(smp_chr_cnt(0), smp_ale_cnt.col(0), mis_ale_cnt.col(0));
  for (arma::uword i = 0; i < pcl_num; i++) {
    for (arma::uword j = 0; j < ptl_smp_hap_cnt.n_cols; j++) {
      wght_tmp(i) = wght_tmp(i) + calculateMultinomProb_arma(ptl_smp_hap_cnt.col(j), smp_chr_cnt(0), part_tmp.col(i));
    }
  }
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    arma::imat ptl_smp_hap_cnt = calculateHaploCount_UnphasedChr_arma(smp_chr_cnt(k), smp_ale_cnt.col(k), mis_ale_cnt.col(k));
    for (arma::uword i = 0; i < pcl_num; i++) {
      ...
      for (arma::uword j = 0; j < ptl_smp_hap_cnt.n_cols; j++) {
        wght_tmp(i) = wght_tmp(i) + calculateMultinomProb_arma(ptl_smp_hap_cnt.col(j), smp_chr_cnt(k), part_tmp.col(i));
      }
    }
...
// Calculate the possible unobserved haplotype counts from the observed haplotype counts and missingness
// [[Rcpp::export]]
arma::imat calculateHaploCount_PhasedChr_arma(const int& smp_chr_cnt, const arma::icolvec& smp_hap_cnt, const arma::icolvec& mis_hap_cnt) {
  // ensure RNG gets set/reset
  RNGScope scope;
  
  arma::imat ptl_smp_hap_cnt_A(4, (mis_hap_cnt(0) + 1) * (mis_hap_cnt(1) + 1));
  for (arma::uword i = 0; i <= mis_hap_cnt(0); i++) {
    for (arma::uword j = 0; j <= mis_hap_cnt(1); j++) {
      ptl_smp_hap_cnt_A(0, j + i * (mis_hap_cnt(1) + 1))  = i;
      ptl_smp_hap_cnt_A(2, j + i * (mis_hap_cnt(1) + 1)) = mis_hap_cnt(0) - i;
      ptl_smp_hap_cnt_A(1, j + i * (mis_hap_cnt(1) + 1)) = j;
      ptl_smp_hap_cnt_A(3, j + i * (mis_hap_cnt(1) + 1)) = mis_hap_cnt(1) - j;
    }
  }
  
  arma::imat ptl_smp_hap_cnt_B(4, (mis_hap_cnt(2) + 1) * (mis_hap_cnt(3) + 1));
  for (arma::uword i = 0; i <= mis_hap_cnt(2); i++) {
    for (arma::uword j = 0; j <= mis_hap_cnt(3); j++) {
      ptl_smp_hap_cnt_B(0, j + i * (mis_hap_cnt(3) + 1)) = i;
      ptl_smp_hap_cnt_B(1, j + i * (mis_hap_cnt(3) + 1)) = mis_hap_cnt(2) - i;
      ptl_smp_hap_cnt_B(2, j + i * (mis_hap_cnt(3) + 1)) = j;
      ptl_smp_hap_cnt_B(3, j + i * (mis_hap_cnt(3) + 1)) = mis_hap_cnt(3) - j;
    }
  }
  
  arma::imat ptl_smp_hap_cnt_AB(4, 1);
  ptl_smp_hap_cnt_AB(0, 0) = mis_hap_cnt(4);
  ptl_smp_hap_cnt_AB(1, 0) = 0;
  ptl_smp_hap_cnt_AB(2, 0) = 0;
  ptl_smp_hap_cnt_AB(3, 0) = 0;
  for (arma::sword i = mis_hap_cnt(4) - 1; i >= 0; i--) {
    for (arma::sword j = mis_hap_cnt(4) - i; j >= 0; j--) {
      for (arma::sword k = mis_hap_cnt(4) - i - j; k >= 0; k--) {
        ptl_smp_hap_cnt_AB.insert_cols(0, 1);
        ptl_smp_hap_cnt_AB(0, 0) = i;
        ptl_smp_hap_cnt_AB(1, 0) = j;
        ptl_smp_hap_cnt_AB(2, 0) = k;
        ptl_smp_hap_cnt_AB(3, 0) = mis_hap_cnt(4) - i - j - k;
      }
    }
  }
  
  arma::imat ptl_smp_hap_cnt(4, ptl_smp_hap_cnt_A.n_cols * ptl_smp_hap_cnt_B.n_cols * ptl_smp_hap_cnt_AB.n_cols);
  for (arma::uword i = 0; i < ptl_smp_hap_cnt_A.n_cols; i++) {
    for (arma::uword j = 0; j < ptl_smp_hap_cnt_B.n_cols; j++) {
      for (arma::uword k = 0; k < ptl_smp_hap_cnt_AB.n_cols; k++) {
        ptl_smp_hap_cnt.col(k + j * ptl_smp_hap_cnt_AB.n_cols + i * ptl_smp_hap_cnt_AB.n_cols * ptl_smp_hap_cnt_B.n_cols) = smp_hap_cnt + ptl_smp_hap_cnt_A.col(i) + ptl_smp_hap_cnt_B.col(j) + ptl_smp_hap_cnt_AB.col(k);
      }
    }
  }
...
// [[Rcpp::export]]
List runBPF_PhasedChr_arma(const double& sel_cof_A, const double& dom_par_A, const double& sel_cof_B, const double& dom_par_B, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::imat& smp_hap_cnt, const arma::imat& mis_hap_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  cout << "generation: " << smp_gen(0) << endl;
  part_tmp = arma::normalise(arma::randu<arma::dmat>(4, pcl_num), 1, 0);
  arma::imat ptl_smp_hap_cnt = calculateHaploCount_PhasedChr_arma(smp_chr_cnt(0), smp_hap_cnt.col(0), mis_hap_cnt.col(0));
  for (arma::uword i = 0; i < pcl_num; i++) {
    for (arma::uword j = 0; j < ptl_smp_hap_cnt.n_cols; j++) {
      wght_tmp(i) = wght_tmp(i) + calculateMultinomProb_arma(ptl_smp_hap_cnt.col(j), smp_chr_cnt(0), part_tmp.col(i));
    }
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    wght_tmp = arma::zeros<arma::dcolvec>(pcl_num);
    part_tmp = part_pst.slice(k - 1);
    arma::imat ptl_smp_hap_cnt = calculateHaploCount_PhasedChr_arma(smp_chr_cnt(k), smp_hap_cnt.col(k), mis_hap_cnt.col(k));
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::dmat path = simulateTLWFDS_arma(sel_cof_A, dom_par_A, sel_cof_B, dom_par_B, rec_rat, pop_siz, part_tmp.col(i), smp_gen(k - 1), smp_gen(k), ptn_num);
      part_tmp.col(i) = arma::vectorise(path.tail_cols(1), 0);
      for (arma::uword j = 0; j < ptl_smp_hap_cnt.n_cols; j++) {
...
// [[Rcpp::export]]
void calculateLogLikelihood_arma(double& log_lik, const double& sel_cof_A, const double& dom_par_A, const double& sel_cof_B, const double& dom_par_B, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::field<arma::imat>& ptl_smp_hap_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  arma::imat smp_hap_cnt = ptl_smp_hap_cnt(0);
  for (arma::uword i = 0; i < pcl_num; i++) {
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    arma::imat smp_hap_cnt = ptl_smp_hap_cnt(k);
    for (arma::uword i = 0; i < pcl_num; i++) {
...
List runPMMH_UnphasedChr_arma(const double& int_sel_cof_A, const double& int_dom_par_A, const double& int_sel_cof_B, const double& int_dom_par_B, const double& int_rec_rat, const int& int_pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::imat& smp_ale_cnt, const arma::imat& mis_ale_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  ...
  arma::field<arma::imat> ptl_smp_hap_cnt(smp_gen.n_elem);
  for (arma::uword k = 0; k < smp_gen.n_elem; k++) {
    ptl_smp_hap_cnt(k) = calculateHaploCount_UnphasedChr_arma(smp_chr_cnt(k), smp_ale_cnt.col(k), mis_ale_cnt.col(k));
  }
  ...
  calculateLogLikelihood_arma(log_lik_chn(0), sel_cof_A_chn(0), dom_par_A_chn(0), sel_cof_B_chn(0), dom_par_B_chn(0), rec_rat_chn(0), pop_siz_chn(0), smp_gen, smp_chr_cnt, ptl_smp_hap_cnt, ptn_num, pcl_num);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    */
    ...
    if (apt_rto == 1) {
      ...
      calculateLogLikelihood_arma(log_lik_chn(i), sel_cof_A_chn(i), dom_par_A_chn(i), sel_cof_B_chn(i), dom_par_B_chn(i), rec_rat_chn(i), pop_siz_chn(i), smp_gen, smp_chr_cnt, ptl_smp_hap_cnt, ptn_num, pcl_num);
...
List runPMMH_PhasedChr_arma(const double& int_sel_cof_A, const double& int_dom_par_A, const double& int_sel_cof_B, const double& int_dom_par_B, const double& int_rec_rat, const int& int_pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::imat& smp_hap_cnt, const arma::imat& mis_hap_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  ...
  arma::field<arma::imat> ptl_smp_hap_cnt(smp_gen.n_elem);
  for (arma::uword k = 0; k < smp_gen.n_elem; k++) {
    ptl_smp_hap_cnt(k) = calculateHaploCount_PhasedChr_arma(smp_chr_cnt(k), smp_hap_cnt.col(k), mis_hap_cnt.col(k));
  }

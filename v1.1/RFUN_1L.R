...
# call C++ functions
sourceCpp("./Code/Code v2.1/Code v2.2/CFUN_1L.cpp")
...
#' Standard version
simulateHMM <- function(model, missing, mis_rat, sel_cof, dom_par, pop_siz, int_frq, smp_gen, smp_siz, ...) {
  int_gen <- min(smp_gen)
  lst_gen <- max(smp_gen)
  ...
  if (missing == TRUE) {
    ...
    for (k in 1:length(smp_gen)) {
      smp_cnt[1, k] <- rbinom(1, size = smp_siz[k], prob = pop_frq[smp_gen[k] - int_gen + 1])
      smp_cnt[2, k] <- smp_siz[k] - smp_cnt[1, k]
      
      smp_cnt[1, k] <- rbinom(n = 1, size = smp_cnt[1, k], prob = 1 - mis_rat)
      smp_cnt[2, k] <- rbinom(n = 1, size = smp_cnt[2, k], prob = 1 - mis_rat)
    }
  } else {
    smp_cnt <- matrix(NA, nrow = 2, ncol = length(smp_gen))
    for (k in 1:length(smp_gen)) {

...
# call C++ functions
sourceCpp("./Code/Code v2.1/Code v2.2/CFUN_2L.cpp")
...
#' Standard version
simulateHMM <- function(model, missing, mis_rat, sel_cof, dom_par, rec_rat, pop_siz, int_frq, smp_gen, smp_siz, ...) {
  int_gen <- min(smp_gen)
  lst_gen <- max(smp_gen)
  ...
  if (missing == TRUE) {
    ...
    for (k in 1:length(smp_gen)) {
      ...
      smp_ale_cnt[3, k] <- smp_hap_cnt[1, k] + smp_hap_cnt[3, k]
      smp_ale_cnt[4, k] <- smp_hap_cnt[2, k] + smp_hap_cnt[4, k]

      smp_ale_cnt[1, k] <- rbinom(n = 1, size = smp_ale_cnt[1, k], prob = 1 - mis_rat)
      smp_ale_cnt[2, k] <- rbinom(n = 1, size = smp_ale_cnt[2, k], prob = 1 - mis_rat)
      smp_ale_cnt[3, k] <- rbinom(n = 1, size = smp_ale_cnt[3, k], prob = 1 - mis_rat)
      smp_ale_cnt[4, k] <- rbinom(n = 1, size = smp_ale_cnt[4, k], prob = 1 - mis_rat)
    }
  } else {
    smp_hap_cnt <- matrix(NA, nrow = 4, ncol = length(smp_gen))
    smp_ale_cnt <- matrix(NA, nrow = 4, ncol = length(smp_gen))

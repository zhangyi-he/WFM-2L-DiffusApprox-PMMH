...
#' Standard version
simulateHMM <- function(model, phased, missing, sel_cof_A, dom_par_A, sel_cof_B, dom_par_B, rec_rat, pop_siz, int_hap_frq, smp_gen, smp_chr_cnt, ...) {
  int_gen <- min(smp_gen)
  lst_gen <- max(smp_gen)
  ...
  if (phased == FALSE) {
    ...
    if (missing == TRUE) {
      mis_ale_cnt <- matrix(NA, nrow = 2, ncol = length(smp_gen))
      ...
      mis_ale_cnt <- matrix(0, nrow = 2, ncol = length(smp_gen))
      smp_hap_cnt <- matrix(NA, nrow = 4, ncol = length(smp_gen))
      smp_ale_cnt <- matrix(NA, nrow = 2, ncol = length(smp_gen))
      for (k in 1:length(smp_gen)) {
        smp_hap_cnt[, k] <- rmultinom(1, size = smp_chr_cnt[k], prob = pop_hap_frq[, smp_gen[k] - int_gen + 1])
    ...
    if (missing == TRUE) {
      ...
      for (k in 1:length(smp_gen)) {
        ...
        smp_hap_cnt[, k] <- smp_hap_cnt[, k] - rmultinom(1, size = min(smp_hap_cnt[, k], mis_hap_cnt[5, k]), prob = rep(0.25, 4))
      }
    } else {
      mis_hap_cnt <- matrix(0, nrow = 5, ncol = length(smp_gen))
      smp_hap_cnt <- matrix(NA, nrow = 4, ncol = length(smp_gen))
      for (k in 1:length(smp_gen)) {
        smp_hap_cnt[, k] <- rmultinom(1, size = smp_chr_cnt[k], prob = pop_hap_frq[, smp_gen[k] - int_gen + 1])
      }
...
#' @param mis_cnt the count of the missing values in the two mutant alleles or the four haplotypes observed in the sample at all sampling time points
#' @param ptn_num the number of subintervals divided per generation in the Euler-Maruyama method
#' @param pcl_num the number of particles generated in the bootstrap particle filter

#' Standard version
runBPF <- function(gen_par, smp_gen, smp_chr_cnt, smp_cnt, mis_cnt, ptn_num, pcl_num) {
  sel_cof_A <- gen_par[1]
  dom_par_A <- gen_par[2]
  sel_cof_B <- gen_par[3]
  dom_par_B <- gen_par[4]
  ...
  if (phased) {
    BPF <- runBPF_PhasedChr_arma(sel_cof_A, dom_par_A, sel_cof_B, dom_par_B, rec_rat, pop_siz, smp_gen, smp_chr_cnt, smp_cnt, mis_cnt, ptn_num, pcl_num)
    ...
    BPF <- runBPF_UnphasedChr_arma(sel_cof_A, dom_par_A, sel_cof_B, dom_par_B, rec_rat, pop_siz, smp_gen, smp_chr_cnt, smp_cnt, mis_cnt, ptn_num, pcl_num)
    ...
    pop_ale_frq_pre_resmp <- array(NA, dim = c(2, pcl_num, length(smp_gen)))
    ...
    pop_ale_frq_pst_resmp <- array(NA, dim = c(2, pcl_num, length(smp_gen)))
...
#' Standard version
runPMMH <- function(int_gen_par, smp_gen, smp_chr_cnt, smp_cnt, mis_cnt, ptn_num, pcl_num, itn_num) {
  ...
  phased <- ifelse(nrow(smp_cnt) == 4, TRUE, FALSE)
  if (phased) {
    PMMH <- runPMMH_PhasedChr_arma(int_sel_cof_A, int_dom_par_A, int_sel_cof_B, int_dom_par_B, int_rec_rat, int_pop_siz, smp_gen, smp_chr_cnt, smp_cnt, mis_cnt, ptn_num, pcl_num, itn_num)
  } else {
    PMMH <- runPMMH_UnphasedChr_arma(int_sel_cof_A, int_dom_par_A, int_sel_cof_B, int_dom_par_B, int_rec_rat, int_pop_siz, smp_gen, smp_chr_cnt, smp_cnt, mis_cnt, ptn_num, pcl_num, itn_num)
  }
...
#' Standard version
runBayesianProcedure <- function(int_gen_par, smp_gen, smp_chr_cnt, smp_cnt, mis_cnt, ptn_num, pcl_num, itn_num, brn_num, thn_num, grd_num) {
  int_sel_cof_A <- int_gen_par[1]
  int_dom_par_A <- int_gen_par[2]
  int_sel_cof_B <- int_gen_par[3]
  int_dom_par_B <- int_gen_par[4]

...
// [[Rcpp::export]]
List runBPF_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::irowvec& smp_ale_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  for (arma::uword i = 0; i < pcl_num; i++) {
    wght_tmp(i) = R::dbinom(smp_ale_cnt(0), smp_chr_cnt(0), part_tmp(i), false);
  }
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      ...
      wght_tmp(i) = R::dbinom(smp_ale_cnt(k), smp_chr_cnt(k), part_tmp(i), false);
    }
...
// [[Rcpp::export]]
void calculateLogLikelihood_arma(double& log_lik, const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::irowvec& smp_ale_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  for (arma::uword i = 0; i < pcl_num; i++) {
    wght(i) = R::dbinom(smp_ale_cnt(0), smp_chr_cnt(0), part_pre(i), false);
  }
  if (arma::sum(wght) > 0) {
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    for (arma::uword i = 0; i < pcl_num; i++) {
      ...
      wght(i) = R::dbinom(smp_ale_cnt(k), smp_chr_cnt(k), part_pre(i), false);
    }
    if (arma::sum(wght) > 0) {
      log_lik = log_lik + (log(arma::sum(wght)) - log(pcl_num));
      arma::dcolvec prob = arma::normalise(wght, 1);
...
//[[Rcpp::export]]
List runPMMH_arma(const double& int_sel_cof, const double& int_dom_par, const int& int_pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_chr_cnt, const arma::irowvec& smp_ale_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  calculateLogLikelihood_arma(log_lik_chn(0), sel_cof_chn(0), dom_par_chn(0), pop_siz_chn(0), smp_gen, smp_chr_cnt, smp_ale_cnt, ptn_num, pcl_num);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    if (apt_rto == 1) {
      ...
      calculateLogLikelihood_arma(log_lik_chn(i), sel_cof_chn(i), dom_par_chn(i), pop_siz_chn(i), smp_gen, smp_chr_cnt, smp_ale_cnt, ptn_num, pcl_num);

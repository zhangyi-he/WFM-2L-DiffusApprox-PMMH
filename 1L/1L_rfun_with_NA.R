...
#' Standard version
simulateHMM <- function(model, missing, sel_cof, dom_par, pop_siz, int_ale_frq, smp_gen, smp_chr_cnt, ...) {
  int_gen <- min(smp_gen)
  lst_gen <- max(smp_gen)
...
#' Standard version
runBPF <- function(gen_par, smp_gen, smp_chr_cnt, smp_ale_cnt, mis_ale_cnt, ptn_num, pcl_num) {
  sel_cof <- gen_par[1]
  dom_par <- gen_par[2]
  pop_siz <- gen_par[3]
  
  # run the bootstrap particle filter
  BPF <- runBPF_arma(sel_cof, dom_par, pop_siz, smp_gen, smp_chr_cnt, smp_ale_cnt, mis_ale_cnt, ptn_num, pcl_num)
...
#' Standard version
runPMMH <- function(int_gen_par, smp_gen, smp_chr_cnt, smp_ale_cnt, mis_ale_cnt, ptn_num, pcl_num, itn_num) {
  int_sel_cof <- int_gen_par[1]
  int_dom_par <- int_gen_par[2]
  int_pop_siz <- int_gen_par[3]
  
  # run the PMMH
  PMMH <- runPMMH_arma(int_sel_cof, int_dom_par, int_pop_siz, smp_gen, smp_chr_cnt, smp_ale_cnt, mis_ale_cnt, ptn_num, pcl_num, itn_num)
...
#' Standard version
runBayesianProcedure <- function(int_gen_par, smp_gen, smp_chr_cnt, smp_ale_cnt, mis_ale_cnt, ptn_num, pcl_num, itn_num, brn_num, thn_num, grd_num) {
  int_sel_cof <- int_gen_par[1]
  int_dom_par <- int_gen_par[2]
  int_pop_siz <- int_gen_par[3]
